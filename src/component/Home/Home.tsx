...
export default function Home() {
  let { addCart, setnumItem,userdata } = useContext(CartContext)
  ...
  async function getAllProducts(queryData: any): Promise<AxiosResponse<Products>> {
    const response = await axios.get(`https://ecommerce.routemisr.com/api/v1/products?page=${queryData.queryKey[1]}`);
      ...
      <div className='container-fluid mt-5'>
        ...
        <nav >
          <ul className="pagination justify-content-center mt-3">
            <li className="page-item"><span className="page-link text-main cursor-pointer" id='1' onClick={getPageNum} >1</span></li>
            <li className="page-item"><span className="page-link text-main cursor-pointer" id='2' onClick={getPageNum} >2</span></li>
          </ul>
        </nav>
      </div>
    </>}</>
